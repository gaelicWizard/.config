...
if [[ -r "/etc/bashrc_${TERM_PROGRAM:-}" ]]; then
	...
	safe_append_prompt_command '_screen_print_dcs_f `update_terminal_cwd`' || {
		echo "general.rc.bash: Unable to manipulate prompt." 1>&2
		return
	}
...
function ll() { CLICOLOR_FORCE="${CLICOLOR:-}" l "$@" | less -e -R; }
...
function locate() {
	...
	local -i NOW="${EPOCHSECONDS:-(date +%s)}" # seconds since EPOCH.
	...
	local DBAGE="$((DBCREATED - MAXAGE))"

	if [[ $DBAGE -gt $NOW ]]; then
...
function ass() {
	if [[ -n "${1:-}" ]] && declare -F isscreen >/dev/null && isscreen; then
